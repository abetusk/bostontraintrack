...
function flushSingleClientData( cli_id )
{
  ...
  for (var bus_id in g_bus_data)
  {
    ...
    try 
    {
      global_connect[ cli_id ].emit( "update:bus", bus_data );
    } catch (ee) {
  ...
  for (var cli_id in global_connect) 
  {
    ...
    try 
    {
      if (global_status[ cli_id ].enable) 
      {
        ...
        global_connect[ cli_id ].emit( "update:bus", vehicle_data );
      }
    } catch (ee) {
  ...
  for (var ind in vehicle){
    ...
    if (bus_id in g_bus_data)
    {
           ...
           (Math.abs(lon - g_bus_data[bus_id].lon) > 0.001) )
      {
        ...
        g_bus_data[bus_id].route_id = route;
        g_bus_data[bus_id].route_name = route;
        g_bus_data[bus_id].route_group = route;
      } else  {
